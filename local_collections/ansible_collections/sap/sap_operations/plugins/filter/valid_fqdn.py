...
- name: This is valid fqdn
  ansible.builtin.assert:
    that: ('hostname13.domain.com' | sap.sap_operations.valid_fqdn)

- name: Valid hostname is also valid fqdn
  ansible.builtin.assert:
    that: ('hostname13' | sap.sap_operations.valid_fqdn)

- name: Valid hostname is also valid fqdn, but it can be up to 63 characters
  ansible.builtin.assert:
    that: ('hostname1234567890123456789012345678901234567890' | sap.sap_operations.valid_fqdn)

- name: Fqdn cannot contain two dots in a sequence
  ansible.builtin.assert:
    that: not ('hostname13..domain.com' | sap.sap_operations.valid_fqdn)
